...
class User(ModelView, ModelSQL):
    ...
    borrowed_book = checkouts = fields.One2Many('library.user.checkout',
        'user', 'Checkouts')
    ...
    def getter_checkedout_books(cls, users, name):
        ...
        if name == "checkedout_books":
            ...
            cond = checkout.user.in_([x.id for x in users]) & \
                (checkout.return_date == Null) & \
                ((checkout.date + datetime.timedelta(days=20))
                    < (datetime.date.today()))
...
class Book(metaclass=PoolMeta):
    ...
    is_available = fields.Function(
        fields.Boolean('Is available', help='If True, at least an exemplary of'
            ' this book is currently available for borrowing', readonly=True),
    ...
    def search_is_available(cls, name, clause):
        ...
        query_table = book.join(exemplary,
            condition=(exemplary.book == book.id)
            ).join(checkout, 'LEFT OUTER',
                condition=(exemplary.id == checkout.exemplary))
...
class Exemplary(metaclass=PoolMeta):
    ...
    def getter_is_available(cls, exemplaries, name):
        # Default case is True, if we find exemplaries in checking without
        # return_date we set it at False
        ...
        cursor.execute(*checkout.select(checkout.exemplary,
                where=(checkout.exemplary.in_([x.id for x in exemplaries])
                    & (checkout.return_date == Null))))

...
class BorrowBook(Wizard):
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({
            'invalid_model': 'This action should be started from a book or an user',
            'invalid_date': 'You cannot checkout books in the future',
            'not_available': 'One of the selected books are not available for booking',
        })
    ...
    def default_choose_book(self, name):    
        if  Transaction().context.get('active_model', '') == 'library.book':
            ...
            for book in books:
                if self._find_free_exemplary(book) == None:
    ...
    def _find_free_exemplary(self, book_id) -> int:
        ...
        cursor.execute(*exemplary.join(checkout, 'LEFT OUTER', condition=(exemplary.id == checkout.exemplary)
                ).select(exemplary.id, 
                    where=(((checkout.return_date != Null) | (checkout.id == Null)) & (exemplary.book == book_id)),
...
class ReturnBook(Wizard):
    ...
    # choose_book = StateView('library.checkout.borrow_book.choose_book',
    #     'library_borrow.create_choose_book_view_form', [
    #         Button('Cancel', 'end', 'tryton-cancel'),
    #         Button('Borrow book', 'create_checkout', 'tryton-go-next'),])
    # create_checkout = StateTransition()
    # open_checkout = StateAction('library_borrow.act_open_user_checkout')
